...
parser.add_argument('--model_save_path', default=os.environ.get('MODEL_SAVE_PATH', './checkpoint'), type=str, help='path to save models and logs')
parser.add_argument('--log_dir', default=os.environ.get('LOG_DIR', './checkpoint'), type=str, help='directory for logs')
...
# Set up model save and log directories
model_save_loc = os.path.join(args.model_save_path, 'TinyImageNet_' + str(args.ratio))
os.makedirs(model_save_loc, exist_ok=True)
log_dir = args.log_dir
os.makedirs(log_dir, exist_ok=True)

# Logging helper
def safe_append_log(filepath, value):
    try:
        with open(filepath, 'a') as f:
            f.write(str(value) + '\n')
    except Exception as e:
        print(f"[Warning] Failed to write to {filepath}: {e}")
...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader):
    ...
    avg_train_loss = loss_x / num_iter
    safe_append_log(os.path.join(model_save_loc, 'train_loss.txt'), avg_train_loss)
...
def test(net1,net2,test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            correct += predicted.eq(targets).cpu().sum().item()                    
    acc = 100.*correct/total
    avg_test_loss = loss_x / max(1, total_batches)
    print("\n| Test Acc: %.2f%%\n" %(acc)) 
    safe_append_log(os.path.join(model_save_loc, 'test_loss.txt'), avg_test_loss)
    safe_append_log(os.path.join(model_save_loc, 'acc.txt'), acc)
    return acc    
...
def find_latest_checkpoint(prefix):
    checkpoint_files = glob.glob(os.path.join(model_save_loc, f"{prefix}_epoch*.pth"))
...
if args.resume:
    model_path_1 = find_latest_checkpoint("Net1")
    model_path_2 = find_latest_checkpoint("Net2")

    if model_path_1 and model_path_2:
        checkpoint1 = torch.load(model_path_1)
        checkpoint2 = torch.load(model_path_2)
        net1.load_state_dict(checkpoint1['net'])
        net2.load_state_dict(checkpoint2['net'])
        if checkpoint1.get('optimizer') is not None:
            optimizer1.load_state_dict(checkpoint1['optimizer'])
        else:
            print("[Warning] Optimizer state not found in Net1 checkpoint")
        if checkpoint2.get('optimizer') is not None:
            optimizer2.load_state_dict(checkpoint2['optimizer'])
        else:
            print("[Warning] Optimizer state not found in Net2 checkpoint")
        start_epoch = checkpoint1.get('epoch', 0) + 1
    else:
        print("[Warning] No latest checkpoint found. Starting from epoch 0.")
        start_epoch = 0
else:
    start_epoch = 0
...
for epoch in range(start_epoch,args.num_epochs+1):   
    ...
    if acc > best_acc[0]:
        if epoch <warm_up:
            model_name_1 = f'Net1_warmup_epoch{epoch}.pth'
            model_name_2 = f'Net2_warmup_epoch{epoch}.pth'
        else:
            model_name_1 = f'Net1_epoch{epoch}.pth'
            model_name_2 = f'Net2_epoch{epoch}.pth'
